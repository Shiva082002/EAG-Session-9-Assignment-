...
class GoogleSearcher:
    ...
    async def search(self, query: str, ctx: Context, max_results: int = 10) -> List[SearchResult]:
        try:
            await self.rate_limiter.acquire()
            await ctx.info(f"Searching Google for: {query}")

            params = {
                "key": self.GOOGLE_API_KEY,
                "cx": self.GOOGLE_CSE_ID,
                "q": query,
                "num": min(max_results, 10),  # max allowed by API
            }
            ...
            for i, item in enumerate(data.get("items", [])):
                ...
                results.append(SearchResult(title=title, link=link, snippet=snippet, position=i + 1))
                if len(results) >= max_results:
                    break
            ...
            await ctx.error("Google search request timed out")
            return []
        except httpx.HTTPError as e:
            await ctx.error(f"HTTP error occurred: {str(e)}")
            return []
...
mcp = FastMCP("google-search")  # [was: "ddg-search"]
searcher = GoogleSearcher()     # [was: DuckDuckGoSearcher()]
fetcher = WebContentFetcher()


@mcp.tool()
async def duckduckgo_search_results(input: SearchInput, ctx: Context) -> str:
    """Search the internet. This tool does not fetch the content of the webpages, it only returns the URLs of the webpages that has the relevant information. Use the tool 'convert_webpage_url_into_markdown' to fetch the content of these relevant webpages.
    Usage: input={{"input": {{"query": "latest AI developments", "max_results": 5}} }} result = await mcp.call_tool('duckduckgo_search_results', input)"""
    try:
