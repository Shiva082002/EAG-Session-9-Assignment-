        ...
        for step in range(max_steps):
            ...
            while lifelines_left >= 0:
                ...
                self.context.memory.add_tool_output(
                    ...
                    tool_result={"result": f"Step {step} || {perception}"},
                    success=True,
                    tags=["perception"],
                )
                ...
                if not selected_tools:
                    log("loop", "⚠️ No tools selected")
                    tool_descriptions = None
                else:
                    log("loop", "Tools selected — sending selected tools to the model")
                    tool_descriptions = summarize_tools(selected_tools, self.mcp)
                # === Planning ===
                ...
                print(f"[plan] {plan}")
                self.context.memory.add_tool_call(
                    tool_name="plan",
                    tool_args={"plan": plan},
                    tags=["plan"],
                )
                    ...
                    if isinstance(result, str):
                        ...
                        if result.startswith("FINAL_ANSWER:"):
                            ...
                            self.context.user_input_override  = (
                                ...
                                f"Your last tool call was:\n"
                                f"==============TOOL CALL==============\n"
                                f"{plan}\n\n"
                                f"==============END OF TOOL CALL==============\n"
                                f"The last tool call produced the below result:\n\n"
                                f"==============START OF OUTPUT OF TOOL==============\n"
                                f"{content}\n\n"
                                f"==============END OF OUTPUT OF TOOL==============\n"
                                f"If the above output has the required information to answer the orignial user query, then infer the data and respond with:\n"
                                f"FINAL_ANSWER: <YOUR__FORMATTED_ANSWER>\n\n"
                                f"Otherwise, return the next FUNCTION_CALL."
                                f"Do not repeat the last tool call with similar parameters."
                            )
